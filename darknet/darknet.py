...
def print_detections(detections, coordinates=False):
    ...
    detections.sort(key = lambda x: (len(x[0]), x[2]))
    #print(type(detections), type(detections[0]))
    print("detections after sort")
    for label, confidence, bbox in detections:
        x, y, w, h = bbox
        if coordinates:
            print("{}: {}%    (left_x: {:.0f}   top_y:  {:.0f}   width:   {:.0f}   height:  {:.0f})".format(label, confidence, x, y, w, h))
...
#    ite=0
#   if len(detections) > 4:
#        for label, confidence, bbox in sorted(detections):
#            if ite == (len(detections) - 1):
#                left, top, right, bottom = bbox2points(bbox)
#                cv2.rectangle(image, (left, top), (right, bottom), colors[label], 1)
#                cv2.putText(image, "{} [{:.2f}]".format('Not Truck', float(confidence)),
#                            (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
#                            colors[label], 2)
#                break
#            elif ite < len(detections):
#                left, top, right, bottom = bbox2points(bbox)
#                cv2.rectangle(image, (left, top), (right, bottom), colors[label], 1)
#                cv2.putText(image, "{} [{:.2f}]".format('x', float(confidence)),
#                            (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
#                            colors[label], 2)
#                ite += 1
#        return image
